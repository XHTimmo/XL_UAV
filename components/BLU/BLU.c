...
#define SAMPLE_DEVICE_NAME "XL_UAV"
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_WRITE_EVT:
        if (!param->write.is_prep)
        {
            ...
            printf("espget:%s\n", param->write.value);
            memcpy(buf,param->write.value,12);
            // b_m.m = buf
            blu_messige.messige = buf;
            printf("espget:%s\n", blu_messige.messige);
            blu_messige.en = 1;
...
BLU_Messige blu_messige;
void BLU_init(void)
{
    esp_err_t ret;
    blu_messige.messige = NULL;
    blu_messige.en = 0;
    /* Initialize NVS. */
    ret = nvs_flash_init();
